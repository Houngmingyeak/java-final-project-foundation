...
public class ClientHandler implements Runnable {
    // បន្ថែម Logger ផ្ទាល់ខ្លួនសម្រាប់ ClientHandler ដើម្បីបំបាត់ Error
    ...
    public void run() {
        try {
            // បង្កើត Output មុន Input ដើម្បីការពារ Deadlock
            ...
            logger.error("Socket error for user {}: {}", username, e.getMessage());
        } finally {
            disconnect();
        }
    }
    ...
    private void handleMessage(Message message) {
        ...
        message.setSender(this.username); // ធានាថាឈ្មោះអ្នកផ្ញើត្រឹមត្រូវ
    ...
    public synchronized void sendMessage(Message message) {
        ...
        try {
            ...
            output.flush();
            output.reset(); // សំខាន់៖ ជម្រះ Cache សម្រាប់ទិន្នន័យរូបភាពថ្មី
        } catch (IOException e) {
            logger.error("Communication error with {}: {}", username, e.getMessage());
            disconnect();
        }
    }
    ...
    private void handleCommand(Command command) {
        switch (command.getType()) {
            ...
            default -> logger.warn("Unknown command: {}", command.getType());
        }
    }
        ...
        if (command.getData() instanceof User credentials) {
            ...
            if (user.isPresent()) {
                ...
                if (server.isUserOnline(uname)) {
                    sendMessage(Message.createSystemMessage("ERROR: User already logged in"));
                    return;
                }
                ...
                if (server.registerClient(username, this)) {
                    ...
                    sendMessage(Message.createSystemMessage("ERROR: Registration failed on server"));
                }
            } else {
